...
# Check if quake_sf exists and has spatial data
if (!exists("quake_sf")) {
  stop("Error: 'quake_sf' object not found. Ensure your earthquake dataset is loaded.")
}

# Ensure quake_sf is an sf object
if (!inherits(quake_sf, "sf")) {
  stop("Error: 'quake_sf' is not an sf object. Convert it using st_as_sf().")
}

# Check CRS of quake_sf
if (is.null(st_crs(quake_sf))) {
  # Assign a default CRS (e.g., WGS 84 - EPSG:4326)
  message("No CRS found for 'quake_sf'. Assigning EPSG:4326 (WGS 84).")
  st_crs(quake_sf) <- 4326
}

# Confirm that CRS assignment is successful
if (is.null(st_crs(quake_sf))) {
  stop("Error: CRS assignment to 'quake_sf' failed.")
}
